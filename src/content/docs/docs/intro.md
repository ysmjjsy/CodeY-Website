---
title: 什么是 CodeY
description: CodeY 是具备持久化执行、显式权限控制与可复用运行时的本地 AI Agent 桌面工作台。
---

CodeY 是一个本地优先的 AI Agent 桌面工作台。它把可靠的 Agent 执行能力放在一个本地守护进程里：任务持久化运行、权限显式受控，同一套运行时既服务桌面端，也通过 SDK 服务你的应用。

:::caution[开发阶段]
CodeY 正处于 `0.1.x` 活跃开发阶段，尚未发布可下载的安装包，请[从源码构建](/docs/getting-started/)。稳定版本发布前，接口和存储格式可能发生变化。
:::

## 两个入口

- **CodeY Desktop**：面向项目的桌面工作台，用于运行、检查和治理本地 Agent 任务。
...
- [快速开始](/docs/getting-started/)：从源码构建并运行 CodeY
